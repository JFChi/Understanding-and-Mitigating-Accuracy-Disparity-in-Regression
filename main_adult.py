...
parser.add_argument("-m", "--model", type=str,
                    ...
                    default="mlp")
parser.add_argument("-y", "--target", help="Name of the target attribute [income]", type=str, 
                    default="income")
parser.add_argument("-p", "--sensitive", help="Name of the sensitive/private attribute", type=str, default="sex")    
parser.add_argument("-c", '--clip', type=float, default=0.005, help="parameters in WassersteinNet")
...
dtype = np.float32
logger.info("--------------------------------------------------")
logger.info("UCI Adult data set, target attribute: {}, sensitive attribute: {}".format(args.target, args.sensitive))
logger.info("seed: {}".format(args.seed))
# Load UCI Adult dataset.
time_start = time.time()
adult_train = AdultDataset(root_dir='data', phase='train', tar_attr=args.target, priv_attr=args.sensitive)
adult_test = AdultDataset(root_dir='data', phase='test', tar_attr=args.target, priv_attr=args.sensitive)
...
num_groups = 2
use_sigmoid = True if args.target == "income" else False
