...
parser.add_argument("-n", "--name", help="Name used to save the log file.", type=str, default="crime")
...
parser.add_argument("-b", "--batch_size", help="Batch size during training", type=int, default=256)
parser.add_argument("-m", "--model", type=str,
					help="Which model to run: [mlp|wmlp|CENet]",
					default="mlp") 
parser.add_argument("-c", '--clip', type=float, default=0.002, help="parameters in WassersteinNet")
...
dtype = np.float32
logger.info("--------------------------------------------------")
logger.info("Running UCI communities and crime data set regression analysis")
logger.info("seed: {}".format(args.seed))
# Load Crime dataset.
time_start = time.time()
crime_train = CrimeDataset(root_dir='data', phase='train')
crime_test = CrimeDataset(root_dir='data', phase='test')
train_loader = DataLoader(crime_train, batch_size=args.batch_size, shuffle=True)
test_loader = DataLoader(crime_test, batch_size=args.batch_size, shuffle=False)
time_end = time.time()
logger.info("Time used to load all the data sets: {} seconds.".format(time_end - time_start))
input_dim = crime_train.xdim
num_groups = 2
use_sigmoid = True
