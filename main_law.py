...
parser.add_argument("-n", "--name", help="Name used to save the log file.", type=str, default="law")
...
parser.add_argument("-r", "--lr", type=float, help="Learning rate of optimization", default=0.2)
parser.add_argument("-b", "--batch_size", help="Batch size during training", type=int, default=128)
...
dtype = np.float32
logger.info("--------------------------------------------------")
logger.info("Running Law School data set regression analysis")
...
train_loader = DataLoader(law_train, batch_size=args.batch_size, shuffle=True)
test_loader = DataLoader(law_test, batch_size=args.batch_size, shuffle=False)
time_end = time.time()
